  ...
  tahmkench: {
    ...
    skills: {
      ...
      q: {
        ...
        cost: 1,
        icon: "➶",
        text: "造成魔法伤害；未满时施加1层品味，满3层时消耗品味并眩晕敌人。",
      },
      w: {
        id: "w",
        key: "W",
  ...
  riven: {
    ...
    skills: {
      ...
      w: {
        ...
        cost: 1,
        icon: "✦",
        text: "造成物理伤害；拥有至少2层符文充能时消耗2层并眩晕敌人。",
      },
      e: {
        id: "e",
        key: "E",
...
const championBuilds = {
  ...
  twistedfate: [
    ...
    { id: "tf_economy", name: "金币循环", items: ["shojin", "cosmic", "hexplate", "nashor", "lichbane", "luden"] },
  ],
};
...
const championGuides = {
  ...
  aphelios: { loop: "A和R各消耗主武器1发弹药，Q消耗1能量与1发弹药并施加副武器印记；Q后用无消耗W切枪，立刻获得副武器Q。", control: "断魄A按实际伤害15%吸血；折镜A获得1枚飞轮、Q固定+3，折镜R将飞轮翻倍并按翻倍数量攻击一次；飞轮无上限。", warning: "初始顺序为通碧→断魄→坠明→荧焰→折镜；每把武器10发弹药。折镜离开主副武器栏时飞轮全部清空；荧焰致盲只影响下一个敌方回合。" },
};
...
const equipment = {
  ...
  shojin: { id: "shojin", name: "朔极之矛", price: 31, image: "/game-icons/shojin.png", ad: 4, hp: 10, tags: ["战士", "技能", "抽牌"], routes: ["darius_fighter", "darius_infinite_r", "tf_economy"], text: "每打出 1 张技能牌，立即抽 1 张牌；手牌达到8张时不再抽取。" },
  ...
  axiom: { id: "axiom", name: "公理圆弧", price: 32, image: "/game-icons/axiom.png", ad: 5, armorPen: 4, tags: ["穿甲", "终结", "能量"], routes: ["jinx_poke", "darius_infinite_r", "riven_combo", "riven_lethality"], text: "每次打出 R 后恢复 1 能量；每场首次触发回手。若 R 会转化为疾风斩，则改为首次疾风斩后让 R 回手。" },
  fimbulwinter: { id: "fimbulwinter", name: "末日寒冬", price: 28, image: "/game-icons/fimbulwinter.png", hp: 18, tags: ["护盾", "控制"], routes: ["tahm_shield"], text: "技能成功打断敌人时获得护盾；基于损失生命的护盾为最大生命4%，其他护盾规则为7%。" },
  ...
  moonstone: { id: "moonstone", name: "月石再生器", price: 26, image: "/game-icons/moonstone.png", ap: 4, hp: 10, tags: ["护盾", "恢复"], routes: ["tahm_shield"], text: "基于损失生命的护盾提高25%并恢复护盾15%生命；基于AD的护盾提高10%并恢复护盾8%生命。" },
  dawncore: { id: "dawncore", name: "黎明核心", price: 34, image: "/game-icons/dawncore.png", ap: 6, tags: ["护盾", "法强"], routes: ["tahm_shield", "tahm_ap"], text: "每件护盾装备使总AP提高6%；基于损失生命的护盾每件提高6%，基于AD的护盾每件提高3%，最多计算2件。" },
...
function Battle({ run, enemyId, onWin, onLose, onQuit }) {
  ...
  const skillPreview = (id, handIndex = -1) => {
    ...
    if (champion.id === "twistedfate") {
      ...
      if (id === "w")
        return "三选一：蓝牌恢复2能量、红牌额外造成伤害、金牌直接眩晕（包括精英与BOSS）；所选牌替换下一张A。";
    ...
    if (champion.id === "tahmkench") {
      ...
      if (id === "q")
        return `造成 ${previewDamage(id, Math.round(8 + level * 4 + hero.ap * 0.8))} 点伤害${foe.taste >= 3 ? "，消耗3层品味并眩晕敌人" : `，品味 ${foe.taste}/3 → ${Math.min(3, foe.taste + 1)}/3`}。`;
      if (id === "w")
    ...
    if (champion.id === "aphelios") {
      ...
      if (id === "attack") {
        ...
        const chakramDamage = main === "calibrum" && marks.calibrum && hero.aphChakrams > 0
          ...
          : 0;
        const attackDamage = previewDamage(id, hero.ad + level * 2 + bonus + chakramDamage);
        const severumDealt = Math.max(0, attackDamage - foe.shield);
        const severumHealing = main === "severum" && severumDealt > 0
          ? Math.min(hero.maxHp - hero.hp, Math.max(1, Math.round(severumDealt * 0.15)))
          : 0;
        return `消耗${mainWeapon.name}1发弹药（剩余${Math.max(0, ammo - 1)}/${APHELIOS_MAX_AMMO}），造成 ${attackDamage} 点伤害${main === "severum" ? `，按实际伤害的15%吸血，恢复${severumHealing}生命` : ""}${bonus ? `，引爆通碧印记并发射${hero.aphChakrams}枚飞轮` : ""}${main === "gravitum" ? "，施加1层坠明印记" : main === "infernum" ? `，施加1层致盲（${foe.apheliosBlind}→${Math.min(5, foe.apheliosBlind + 1)}/5，下一个敌方回合落空率${Math.min(100, (foe.apheliosBlind + 1) * 20)}%，随后清空）` : main === "crescendum" ? `，获得1枚折镜飞轮（${hero.aphChakrams}→${hero.aphChakrams + 1}，无上限）${ammo === 1 ? "；折镜随后离场，全部飞轮清空" : ""}` : ""}。`;
      }
      if (id === "q") {
        ...
        const crescendumBaseRaw = 8 + hero.ad * 0.65 + level * 3;
        const perChakramDamage = Math.max(0, previewDamage(id, Math.round(crescendumBaseRaw + 4 + hero.ad * 0.25)) - previewDamage(id, Math.round(crescendumBaseRaw)));
        const offText = main === "calibrum" ? `施加通碧与${offWeapon.name}印记` : main === "severum" ? `恢复${severumHealing}生命并施加${offWeapon.name}印记×3` : main === "gravitum" ? (marks.gravitum ? "消耗1层坠明印记并眩晕" : "需要坠明印记才能眩晕") : main === "infernum" ? `施加1层致盲（${Math.min(5, foe.apheliosBlind + 1)}/5，下一个敌方回合落空率${Math.min(100, (foe.apheliosBlind + 1) * 20)}%，随后清空）与${offWeapon.name}印记` : `驱动${hero.aphChakrams}枚飞轮（每枚追加${perChakramDamage}点伤害）并施加${offWeapon.name}印记；飞轮固定+3（${hero.aphChakrams}→${hero.aphChakrams + 3}，无上限）${ammo === 1 ? "；折镜随后离场，全部飞轮清空" : ""}`;
      ...
      if (main === "gravitum") effectText = "定身敌人1个敌方回合";
      if (main === "infernum") effectText = "叠满5层致盲，下一个敌方回合落空率达到100%，随后清空";
      if (main === "crescendum") {
        const doubledChakrams = hero.aphChakrams * 2;
        const perChakramRaw = Math.round(4 + hero.ad * 0.25);
        const rBaseWithoutChakrams = 14 + hero.ad + level * 4 + markBurst;
        const perChakramDamage = Math.max(0, previewDamage(id, Math.round(rBaseWithoutChakrams + perChakramRaw)) - previewDamage(id, Math.round(rBaseWithoutChakrams)));
        raw += doubledChakrams * perChakramRaw;
        effectText = `飞轮翻倍（${hero.aphChakrams}→${doubledChakrams}，无上限），随后以${doubledChakrams}枚飞轮各追加${perChakramDamage}点伤害攻击一次${ammo === 1 ? "；折镜随后离场，全部飞轮清空" : ""}`;
      }
      return `强化月蚀：消耗${mainWeapon.name}1发弹药（剩余${Math.max(0, ammo - 1)}/${APHELIOS_MAX_AMMO}），引爆${totalMarks}层全部印记（爆破基础伤害+${markBurst}），造成 ${previewDamage(id, Math.round(raw))} 点伤害；${effectText}。`;
    }
    if (champion.id === "riven") {
      ...
      if (id === "w") {
        ...
        return `造成 ${previewDamage(id, Math.round(8 + hero.ad * 0.65 + level * 4))} 点伤害${charged ? "，消耗2层充能并眩晕敌人" : `；需要2层符文充能才能控制（当前 ${hero.runeCharge}/2）`}；结算后获得1层符文充能。`;
      }
      if (id === "e") {
        const shieldItems = run.gear.filter((gearId) => equipment[gearId].tags.includes("护盾")).length;
    ...
    if (id === "q")
      return `造成 ${damage} 点伤害并施加眩晕；敌人本回合无法行动。`;
  ...
  const play = (id, index, chosenCard = null) => {
      ...
      if (id === "attack") {
        ...
        if (h.empowered) {
          ...
          if (h.selectedCard === "金牌") {
            const result = attemptStun(f);
            message += result === "stunned" ? " 金牌造成眩晕！" : " 控制被免疫。";
          }
          h.empowered = false;
          h.selectedCard = null;
        }
    ...
    if (champion.id === "aphelios") {
      ...
      if (id === "attack") {
        ...
        if (main === "infernum") { f.apheliosBlind = Math.min(5, (f.apheliosBlind || 0) + 1); message += ` 荧焰施加1层致盲（下一个敌方回合落空率${f.apheliosBlind * 20}%）。`; }
        if (main === "crescendum") { h.aphChakrams += 1; message += ` 获得1枚折镜飞轮（当前${h.aphChakrams}，无上限）。`; }
      }
      if (id === "q") {
        consumeAmmo(1);
        ...
        if (main === "gravitum") {
          ...
          if (consumeMark("gravitum")) {
            const result = attemptStun(f, 2);
            message += result === "stunned" ? " 暗蚀消耗坠明印记并眩晕敌人！" : result === "immune" ? " 暗蚀消耗坠明印记，但控制被免疫。" : " 暗蚀消耗坠明印记。";
          } else message += " 当前没有坠明印记，暗蚀无法眩晕。";
          addMark(off);
        }
        if (main === "infernum") { damage = Math.round(7 + h.ad * 0.6 + level * 2.5); f.apheliosBlind = Math.min(5, (f.apheliosBlind || 0) + 1); addMark(off); message += ` 暝涌施加1层致盲（下一个敌方回合落空率${f.apheliosBlind * 20}%），并施加${offName}印记。`; }
        if (main === "crescendum") {
          const count = h.aphChakrams;
          damage = Math.round(8 + h.ad * 0.65 + level * 3 + count * (4 + h.ad * 0.25));
          addMark(off, count > 0 ? Math.min(3, count) : 1);
          h.aphChakrams = count + 3;
          if (count > 0) message += ` 驻灵驱动${count}枚飞轮命中并施加${offName}印记；结算后飞轮固定+3，当前${h.aphChakrams}。`;
          else message += ` 驻灵施加1层${offName}印记并获得3枚飞轮。`;
        }
      }
      if (id === "w") {
      ...
      if (id === "r") {
        ...
        if (main === "gravitum") { f.stunned = true; f.apheliosRootTurns = 1; message += " 坠明将敌人定身1个敌方回合。"; }
        if (main === "infernum") { f.apheliosBlind = 5; message += " 荧焰叠满5层致盲（下一个敌方回合落空率100%，随后清空）。"; }
        if (main === "crescendum") {
          h.aphChakrams *= 2;
          const volley = h.aphChakrams * Math.round(4 + h.ad * 0.25);
          damage += volley;
          message += ` 折镜飞轮翻倍至${h.aphChakrams}枚，并以翻倍后的数量攻击一次（飞轮基础伤害+${volley}）。`;
        }
      ...
      if ((id === "attack" || id === "q" || id === "r") && (h.aphAmmo[h.aphMainWeapon] || 0) === 0) {
        ...
        if (previousMain === "crescendum" && h.aphChakrams > 0) {
          message += ` 折镜离开当前武器栏，${h.aphChakrams}枚飞轮全部消失。`;
          h.aphChakrams = 0;
        }
    ...
    if (damage > 0) {
      ...
      if (severumBasicAttack && result.dealt > 0) {
        const healing = Math.min(h.maxHp - h.hp, Math.max(1, Math.round(result.dealt * 0.15)));
        h.hp += healing;
        message += ` 断魄普攻吸血恢复 ${healing} 点生命。`;
      }
      if (criticalDamage > 0) {
      ...
      <section className="battlefield">
        ...
        <div className="combatant enemy-combatant">
          ...
          <div className="unit-stats">
            ...
            {champion.id === "aphelios" && foe.apheliosBlind > 0 && <span className="enemy-debuff">☄ 致盲 {foe.apheliosBlind}/5 · 本敌方回合落空率 {foe.apheliosBlind * 20}%</span>}
          ...
          <div className="card-choice-panel">
            ...
            <div className="pick-a-card-grid">
              ...
              <button className="pick-gold" onClick={() => { play("w", cardChoice.index, "金牌"); setCardChoice(null); }}>
                <b>金牌</b><span>下一张A直接施加眩晕；精英/BOSS同样一次生效</span>
              </button>
            </div>
...
const attributePoolFor = (run, source) => {
  ...
  const choices = [
    ...
    { id: "gold", icon: "◈", name: "商路收益", text: `立即获得 ${[6, 8, 10][chapter]} 金币`, selectionWeight: 0.35 },
  ];
  if (source === "camp") {
    choices.push(
...
const attributeChoicesFor = (run, source) => {
  const pool = attributePoolFor(run, source);
  const recent = new Set((run.attributeHistory || []).slice(-4));
  const freshPool = pool.filter((choice) => !recent.has(choice.id));
...
const applyAttributeChoice = (run, choice) => {
  ...
  if (choice === "gold") gold += [6, 8, 10][chapter];
  return {
    ...run,
    hero,
    gold,
    attributeHistory: [...(run.attributeHistory || []), choice].slice(-4),
  };
};
